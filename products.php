...
// آرایه دسته‌بندی‌های مدنظر
$allowed_categories = [14, 15, 16, 17, 18];

// آماده‌سازی placeholderها برای IN clause
$placeholders = implode(',', array_fill(0, count($allowed_categories), '?'));

// آماده‌سازی کوئری محصولات
$products_stmt = $db->prepare("SELECT * FROM product WHERE category_id IN ($placeholders) LIMIT 6");
$products_stmt->execute($allowed_categories);
    ...
    <div class="hero-container">
      <div class="hero-content">
        ...
        <p>
          ...
          با جستجو در سایت،
          <br> به انواع کتاب‌ها دسترسی پیدا کنید.
        </p>
      </div>
        ...
        <div class="products-grid">
          ...
          if ($products_stmt->rowCount() > 0) {
            foreach ($products_stmt as $product) {
              ...
              // ✅ امن‌سازی در کوئری (بهتر است با prepare انجام شود، اما برای سازگاری موقت این‌گونه می‌ماند)
              $product_category = $db->query("SELECT * FROM categories WHERE id = " . (int)$cat_id)->fetch();
              ?>
              <article class="product-card">
                <img src="./upload/products/<?php echo htmlspecialchars($product['pic'], ENT_QUOTES, 'UTF-8'); ?>" 
                ...
                <div class="product-body">
                  <header class="product-header">
                    ...
                    <span class="product-category">
                      <?php echo $product_category ? htmlspecialchars($product_category['title'], ENT_QUOTES, 'UTF-8') : 'دسته‌بندی نامشخص'; ?>
                    </span>
                  </header>
                  ...
                  <a href="single_product.php?product=<?php echo (int)$product['id']; ?>" class="btn-view-post">خرید محصول</a>
                </div>
              </article>
            <?php
            }
